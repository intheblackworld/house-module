<?php
    ...
    $db_user = 'htw';
    $db_pass = '748aSgl5Ni';
    $db_name = 'Lix800.lix35tw';

    $con = mysql_connect($db_host, $db_user, $db_pass);
    mysql_query("SET NAMES UTF8");
    mysql_select_db($db_name, $con);

    $query = "SELECT tomail FROM susers WHERE email = 'by'";
...
<?php
    ...
    $mail->Body = "網站：https://by.h35.tw/<BR>姓名：".$name."<BR>電話：".$phone."<BR>信箱：".$user_email."<BR>城市：".$city.$area."<BR>留言：".$msg."<BR><BR>填表日期：".$datetime."<BR>廣告來源：".$utm_source."<BR>廣告媒介：".$utm_medium."<BR>廣告名稱：".$utm_campaign."<BR>廣告內容：".$utm_content; //設定郵件內容 
