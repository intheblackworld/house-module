...
new Promise((resolve, reject) => {
  ...
  fs.access(SITE_ROOT, fs.F_OK, (err_1) => {
    if (err_1) {
      if (err_1.code !== 'ENOENT') return reject(err_1)
      ...
      return ncp(`${SEMANTIC_ROOT}/_site`, SITE_ROOT, (err_2) => {
      ...
      fs.access(file, fs.F_OK, (err) => {
        if (err) {
          ...
          content = content.replace(
            /(@themesFolder\s*\:\s*['"]).*(['"])/,
            `$1${prefix + SEMANTIC_ROOT}/themes$2`,
          )
          content = content.replace(
            /(@siteFolder\s*\:\s*['"]).*(['"])/,
            `$1${prefix + SITE_ROOT}$2`,
          )
    ...
    () => new Promise((resolve, reject) => {
      ...
      fs.access(file, fs.F_OK, (err) => {
        if (err) {
          ...
          fs.writeFileSync(file, `@import "../../${SITE_ROOT}/theme.config";\n`, 'utf8')
        }
        resolve()
        return false
      })
  ...
  .then(() => {
    ...
    return find('../../themes/', `${SEMANTIC_ROOT}/themes`, /\.(variables|overrides)$/).then(
      (results) => {
        for (const filename in results) {
          const content = fs.readFileSync(filename, 'utf8')
          if (REG_FONT_PATH.test(content)) {
            console.log('  Updating font paths inside', filename)
            const newContent = content.replace(REG_FONT_PATH, '$1../$2')
